...
function sendMessage(to_user_id) {
	// Lấy giá trị của to_user_id từ phần tử contact đang được chọn
    var to_user_id = $('.contact.active').data('touserid');
	...
	$.ajax({
		...
		error: function (jqXHR, textStatus, errorThrown) {
            console.error("Error sending message:", jqXHR, textStatus, errorThrown);
		}
	});	
}*/
function sendMessage(to_user_id) {
    var message = $('.message-input input').val().trim();
    if (message == '') {
        return false;
    }
    $('.message-input input').val(''); // Clear the input after capturing the value
...
function updateUserChat() {
    $('li.contact.active').each(function() {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.error("Error updating chat:", textStatus, errorThrown);
            }
        });
    });
}
